function [MMSE_SIC_BER] = MMSE_SIC_Based_MIMO_Simulation(M, SNR, N_Iter, Modulation_Order)
...
MMSE_SIC_BER_num = zeros((2^Modulation_Order)^(M-1), 9);
for num00 = 1:(2^Modulation_Order)^(M-1)
    ...
    for num0=1:9 % size(SNR) = (1,9)
        ...
        for num1=1:N_Iter
            ...
            post_processed_r = MMSE_SIC_Calculate(M, SNR, Hsorted, ssorted, n, num0);
        ...
        MMSE_SIC_BER_num(num00, num0) = error_cnt(num0)/(N_Iter*M*2);
    end
end

MMSE_SIC_BER = sum(MMSE_SIC_BER_num) / (2^Modulation_Order)^(M-1);