...
if Modulation_Order == 2
    s = Number_Of_Cases(M, Modulation_Order);       % s 생성
    const_s = (1/sqrt(2))*Modulation_map(Modulation_Order);     % QPSK stmobl 생성
...
% Number of 16 Cases in s (비교할 s의 경우의 수)
s_NoC = Number_Of_Cases(M, Modulation_Order);

ML_BER_num = zeros((2^Modulation_Order)^M, 9);

for num00 = 1:(2^Modulation_Order)^M
    s_num = s(:, num00);
...
% 
% %                     writematrix(csv_data, 'MIMO_2_0dB_NoC_16.csv', 'WriteMode', 'append');       % Set train data
% 
%                     if rem(num3, 5) == 0
%                         writematrix(csv_data, 'MIMO_2_0dB_test_NoC_16.csv', 'WriteMode', 'append');      % Set test data
%                     end
% 
%                 elseif M == 4
%                     csv_data = [num0, ...
...
% 
% %                     writematrix(csv_data, 'MIMO_4_0dB_NoC_256.csv', 'WriteMode', 'append');       % Set train data
% 
%                     if rem(num3, 5) == 0
%                         writematrix(csv_data, 'MIMO_4_0dB_NoC_256_test.csv', 'WriteMode', 'append');      % Set test data
%                     end
%                 end
%             end
...
ML_BER = sum(ML_BER_num) / (2^Modulation_Order)^M;